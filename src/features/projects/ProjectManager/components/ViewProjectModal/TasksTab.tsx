...
import { fetchTimesheetTasks } from '@/stores/slices/timesheetSlice';
import {
  selectTimesheetTasks,
  selectTimesheetTasksError,
  selectTimesheetTasksLoading,
...
export const TasksTab: React.FC<TasksTabProps> = ({
  ...
  const items = useSelector(selectTimesheetTasks);
  const { totalMin, billMin, percentage, rows } = useStats(items, {
    getId: (item) => item.taskId,
    getName: (item) => item.taskName,
    getTotalMinutes: (item) => item.totalWorkingTime,
    getBillableMinutes: (item) => item.billableWorkingTime,
  });
  const loading = useSelector(selectTimesheetTasksLoading);
  const error = useSelector(selectTimesheetTasksError);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(getErrorMessage(e, 'Failed to load task stats'));
    }
